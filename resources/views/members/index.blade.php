<x-layouts.app title="Mitglieder">
    <div class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
            ...
            <p class="mt-1 text-sm text-slate-500">Mitgliederstammdaten, Status und Organisationszuordnungen verwalten.</p>
        </div>
    ...
    <form method="get" class="cv-panel mt-6 grid gap-3 p-4 md:grid-cols-[minmax(0,2fr)_180px_minmax(220px,1fr)_auto]">
    ...
    <section class="cv-panel mt-4 overflow-hidden">
        <div class="overflow-x-auto">
            <table class="w-full min-w-[900px] text-left text-sm">
                <thead class="bg-slate-50 text-xs uppercase tracking-wide text-slate-500"><tr><th class="px-5 py-3">Mitglied</th><th class="px-5 py-3">Nr.</th><th class="px-5 py-3">Status</th><th class="px-5 py-3">Organisation</th><th class="px-5 py-3">Eintritt</th><th class="px-5 py-3 text-right">Aktion</th></tr></thead>
                ...
                @forelse($members as $member)
                    <tr class="hover:bg-slate-50/80">
                        ...
                        <td class="px-5 py-4 text-slate-600">{{ $member->memberships->where('is_primary', true)->first()?->organizationUnit?->name ?? $member->memberships->first()?->organizationUnit?->name ?? 'Nicht zugeordnet' }}</td>
                ...
                @empty
                    <tr><td colspan="6" class="px-5 py-12 text-center"><p class="font-semibold">Keine Mitglieder gefunden</p><p class="mt-1 text-sm text-slate-500">Passen Sie die Filter an oder legen Sie das erste Mitglied an.</p></td></tr>
                @endforelse
                </tbody>
            </table>
        </div>
