@php
    $currentAnswers = old('answers', []);
@endphp

<div class="space-y-5" data-dynamic-form>
    @foreach($form->fields as $field)
        @php
            $conditionJson = $field->condition
                ? json_encode($field->condition, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES)
                : '';
            ...
            $answerErrorKey = 'answers.'.$field->field_key;
            $fileErrorKey = 'files.'.$field->field_key;
        @endphp
        ...
        >
            @if($field->field_type === 'heading')
                <div class="border-b border-slate-200 pb-2">
                    <h2 class="text-xl font-bold text-slate-900">{{ $field->label }}</h2>
                    @if($field->help_text)
            ...
            @elseif($field->field_type === 'info')
                <div class="rounded-xl border border-blue-200 bg-blue-50 p-4">
                    <p class="font-semibold text-blue-950">{{ $field->label }}</p>
                    @if($field->help_text)
                        <p class="mt-1 whitespace-pre-line text-sm text-blue-900/80">{{ $field->help_text }}</p>
                    @endif
                </div>
            @else
                <label class="cv-label" for="field-{{ $field->id }}">
                    {{ $field->label }}
                ...
                @elseif($field->field_type === 'checkbox')
                    ...
                    <label class="mt-2 flex items-start gap-3 rounded-lg border border-slate-200 px-4 py-3">
                        <input
                            id="field-{{ $field->id }}"
                            type="checkbox"
                            name="answers[{{ $field->field_key }}]"
                ...
                @elseif($field->field_type === 'multiselect')
                    ...
                    >
                        @foreach($options as $option)
                            <option
                                value="{{ $option['value'] }}"
                                @selected(in_array($option['value'], (array) $value, true))
                            >
                                {{ $option['label'] }}
                            </option>
                        @endforeach
                ...
                @if($field->help_text)
                    <p class="mt-1.5 text-xs text-slate-500">{{ $field->help_text }}</p>
                @endif

                @if($errors->has($answerErrorKey))
                    <p class="mt-1 text-sm text-red-700">{{ $errors->first($answerErrorKey) }}</p>
                @endif

                @if($errors->has($fileErrorKey))
                    <p class="mt-1 text-sm text-red-700">{{ $errors->first($fileErrorKey) }}</p>
                @endif
            @endif
        </div>
    @endforeach
...
(() => {
    ...
    const values = () => {
        ...
        blocks.forEach(block => {
            ...
            const inputs = [...block.querySelectorAll('input,select,textarea')]
                .filter(el => !el.disabled && el.type !== 'file' && el.type !== 'hidden');
    ...
    const refresh = () => {
        ...
        blocks.forEach(block => {
            ...
            let condition;
            try {
                condition = JSON.parse(block.dataset.condition);
            } catch (_) {
                return;
            }
